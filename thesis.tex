...
%%% COVER: Settings %%%
\title{Learning Symbolic Latent Representations for Relational Data}
%\subtitle{Multiscale computing for Dummies}

\author{Sebastijan}{Dumančić}

\supervisor{Prof.~dr.~ir.~ Hendrik~Blockeel}{}

\president{Prof.~dr.~Carlo~Vandecasteele}
\jury{Prof.~dr.~ir.~Jesse~Davis\\
      Prof.~dr.~ir.~Johan~Suykens
}
\externaljurymember{Prof.~dr.~David~Poole}{University of British Columbia, Canada}
\externaljurymember{Dr.~Mathias~Niepert}{NEC Labs Europe, Germany}
\externaljurymember{Prof.~dr.~Sebastian~Riedel}{University College London, United Kingdom}


% Your research group within the department
% e.g. iMinds-DistriNet, Scientific Computing Group, ...
\researchgroup{Declarative Languages and Artificial Intelligence}
\website{https://sebdumancic.github.io/} % Leave empty to hide
\email{sebastijan.dumancic@cs.kuleuven.be} % Leave empty to hide
...
\newglossaryentry{alp}{name={Alp},description={Auto-encoding Logic Program}}
\newcommand{\alp}[1]{\gls{alp}#1}
...
\includechapter{learninglogic}
\includechapter{learningrepresentations}
\includechapter{clustering}
\includechapter{symmetries}
\includechapter{autoencoders}
\includechapter{embeddinganalysis}
...
\bibliography{allpapers,mypapers,starai}
