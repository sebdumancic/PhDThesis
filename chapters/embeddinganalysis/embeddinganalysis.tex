...
%As outlined in Chapter 3, recent years have yielded a novel \textit{distributional} paradigm within the \gls{srl} community, \textit{Knowledge graph embeddings} (KGEs)~\cite{EmbeddingsOverview} inspired by the recent success of deep neural networks and representation learning.
...
\gls{kge} methods are usually trained to assign high score to all facts in a knowledge base, not to specific \textit{target} facts.
