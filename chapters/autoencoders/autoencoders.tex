...
%\tias{is the exactly here not a it weird, as we also learn approximate, but still at the relation level? 'can keep data at relational level'?}
...
%\seb{this is maybe downplaying the benefits of SRL}


We argue that retaining first-order logic as a data representation language within DL approaches offers several benefits. 
...
%\tias{is DL the black box, or the KGE?}
%\seb{both!}
...
We introduce \textit{auto-encoding logic programs} (\alp{s}) -- a novel relational DL component which uses logic programs as a computation engine instead of neural networks.
...
\alp{s} further relates to the problem of \textit{predicate invention} \cite{Kramer1995,Muggleton2015,Cropper2018,Kok2007} within SRL -- the problem of inventing a new relational vocabulary defined in terms of the provided one. 
...
\begin{figure*}
	\centering
    \includegraphics[width=.9\textwidth]{alps}
...
%\tias{in the fig, latent 1 uses a semicolon which is an or which is not explained here. Not knowing this makes the example nonunderstandable}).
...
%\begin{align}
%    \min & \sum_{e \in \mathcal{KB}} \mathbb{1}[e \not\in \mathcal{D}(\mathcal{E}(\mathcal{KB}))]  \\
%         & + \sum_{e \in \mathcal{HB(P,C)} \wedge e \not\in \mathcal{KB}}  \mathbb{1}[e \in \mathcal{D}(\mathcal{E}(\mathcal{KB}))]
%    \label{eq:reconstruction}
%\end{align}
...
%\tias{Note that because $\gamma G$ is constant you can rewrite this into: $\sum_{i=1}^{N}w_i\text{\texttt{l}}_i - \sum_{i=1}^N\gamma G\text{\texttt{l}}_i \leq 0 \Leftrightarrow \sum_{i=1}^{N}(w_i -\gamma G)\text{\texttt{l}}_i \leq 0$ which is much more efficient to propagate...}
%seb{did this in the implementation, thought this would be easier to understand in the paper}
...
%\textit{Generality of clauses.} A clause $c_1$ is more general than a clause $c_2$ if all of the examples entailed by $c_2$ are also entailed by $c_1$.
%As $c_2$ cannot bring new information if $c_1$ is also a part of the solution, we impose the constraint stating the not both of the clauses can be selected at the same time:
%
%\begin{center}
%    !(\texttt{c}$_1$ $\wedge$ \texttt{c}$_2$) == \texttt{true}.
%\end{center}
...
It imposes a limit on the number of backtracks for individual runs of a complete search; once the limit is reached, it restarts the search in a random neighbourhoud around the current best solution. % a random part of the search space and explores its neighbourhood.
...
%It is important to add insightful and non-trivial constraints that can prune the search space and significantly improve the scalability of \alp{s}.
%Therefore, we can prune the search space by removing all uninformative candidates, which in turn significantly helps the search.
%To do that, we introduce three novel strategies for reducing the number of candidate clauses.
%\wannes{You make this look too easy. State that it is important to add insightful/non-trivial constraints that can prune the search space.}
...
%\wannes{I dropped the bodies. It made it look like an example while it wasn't. And the bodies weren't used in the definition.}

%\wannes{Is this definition identical to saying that two clauses are naming variants if the heads are true for the exact same set of constants?}
...
%\wannes{Except that one talks about encoder and the other about decoders, def 7 is a special case of def 6. What is the intuition why we need a more strict version for the decoder vs the more loose version for the encoder? E.g. def 6 avoids creating interchangeable latent atoms which do not have a specific meaning anyway while def 7 ... (I cannot come up with an intuition)}
...
%\tias{You don't use Fig 1 very much. Perhaps relate to it a few times in this text as well? e.g. encoding, lp, decoding lp, what is the loss here (well, 0, but OK...) Can also help to point out language bias/bg here (if applicable)}

%\seb{Add more detailed example and explanation of language bias $\rightarrow$ put in the supplementary material}

% Intuitively, this can be seen in the following way. 
% The language bias provided by the user specifies all encoder clauses that can be used by specifying all logical formulas that can be used as bodies of encoder clauses.
% Having the candidate encoder clauses, all decoder clauses are specified accordingly.
% The problem is now defined as selecting a subsets of decoder clauses that give the best reconstruction.
...
\begin{figure*}
    \begin{minipage}[thb]{.33\linewidth}
        \centering
        \includegraphics[width=.9\linewidth]{webkb_AUCPR_final}
    \end{minipage}
    \begin{minipage}[htb]{.33\linewidth}
        \centering
        \includegraphics[width=.9\linewidth]{cora_AUCPR_final}
    \end{minipage}
    \begin{minipage}[htb]{.33\linewidth}
        \centering
        \includegraphics[width=.9\linewidth]{uwcse_AUCPR_final}
    \end{minipage}
...
%\begin{displayquote}
%\textit{Does inducing a latent latent representation with \alp{s} help in learning better models?}
%\end{displayquote}
%\tias{bit vague because meaning of model not that clear; for solving SRL tasks?}
...
To learn the \textit{latent} MLN model, we first learn the latent representation with \alp{s} varying the length of clauses and the compression level, and learn an MLN on the resulting latent representation.
...
%Note that this is not as limiting as it seems at the first sight, if more complexity is necessary it can be achieved by \textit{stacking} several layers of \alps.
%Even if latent representation captures such short relational patterns, it can be very useful in an SRL setting: imagine that in order to successfully model the target domain, formulas of length 6 are necessary.
%A learner learning from the given data representation would have to search a huge space to find such a formula.
%However, if latent representation captures only "short" patterns of length 3, only 2 should latent predicates are enough to represent the initial formula requiring 6 predicates. \wannes{This paragraph is confusing, alternative proposal below}
...
Figure \ref{fig:resultsAUCPR} summarises the performance, in terms of AUC-PR scores, of MLNs learned on the original data representation and various representation created by \alp{s} by modifying the clause lengths for both encoder and decoder, and varying the compression level of the latent representations.
...
The results for the UWCSE dataset show that the performance gain is marginal, improved by only 0.05 points, and most of the latent MLNs perform \textit{worse} than the baseline MLNs.
...
\alp{s} differ in the way that they retain full semantics of logic and use the programming language for mapping to the latent space, instead of purely algebraic manipulations.
...
Rockt\"{a}schel and Riedel (\cite{DTP2017}) integrate theorem proving techniques with the embeddings representations and, in contrast to \alp{s}, learn embeddings representation of relational data.
...
%However, these methods focus on predictive learning only.
...
This work introduce \textit{Auto-encoding Logic Programs} (\alp{s}) -- a novel representation learning framework for representation learning with relational data.
